...
Tables
------
line-line features from user selection
lines_all-all line features for omotting
lines_bon-line features in user selected boundary for omitting
*/
$time=microtime(true);
...
$sq_len=200.0;   //length of side of each grid
...
{	/////////////////
			...
			{	
				if(count($places)==0)return true;
				$result3=pg_query($GLOBALS['db_connection'],
						"select array_to_json(s.tagsarr) from nodes s where 
	...
	for($lat=$bottom;$lat<=$top;$lat=$lat+$deg_lat)
		for($lng=$left;$lng<=$right;$lng=$lng+deg_lng($lat))	
			...
			$str1=$str1."(ST_SetSRID(ST_Point(".$lng.",".$lat."),4326),array[".$lat.",".$lng."]),";
	
	log_time("check");////////////////////////////////////////////////////////////////////
	
	
	if($str1=="") return $selected;
	$str=$str.$str1;
	$str[strlen($str)-1]=';';
	$result_pts = pg_query($GLOBALS['db_connection'],$str);

	if($tables!="")
		$result5=pg_query($GLOBALS['db_connection'],
					"delete from points where chk_intersects_dwith_geom(geom,".$GLOBALS['buff'].")=true;
					delete from points where chk_layers(array[".$tables."],array[".$tables_val."],geom)=false;
					select latlng[1],latlng[2] from points;");
...
{	$a=$_POST["id"];
	...
	{	
		...
		$tem=array();
		array_push($tem,array("lat"=>13.116369576168,"lng"=>80.104136340475));
		array_push($tem,array("lat"=>13.116369576168,"lng"=>80.108351166348));
		array_push($tem,array("lat"=>13.116369576168,"lng"=>80.110458579284));
		array_push($tem,array("lat"=>13.118178319634,"lng"=>80.106269527054));
		array_push($tem,array("lat"=>13.118178319634,"lng"=>80.110489039043));
		array_push($tem,array("lat"=>13.1199870631,"lng"=>80.099959100604));
		array_push($tem,array("lat"=>13.1199870631,"lng"=>80.102071211791));
		array_push($tem,array("lat"=>13.1199870631,"lng"=>80.104183322977));
		array_push($tem,array("lat"=>13.1199870631,"lng"=>80.106295434164));
		array_push($tem,array("lat"=>13.1199870631,"lng"=>80.108407545351));
		array_push($tem,array("lat"=>13.1199870631,"lng"=>80.110519656537));
    ...
    <div id="wrapper">
        ...
        <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
            ...
            <div class="navbar-header">
                ...
                <a class="navbar-brand headtitle" href="index.php">Land Selection App</a>
            </div>
            <!-- Top Menu Items -->
            <?php /*
            ...
            <div class="collapse navbar-collapse navbar-ex1-collapse">
                <ul class="nav navbar-nav side-nav" >
                    <li class="active">
                        <a href="index.php"><i class="fa fa-fw fa-dashboard"></i> Map</a>
                    </li>
                    <li>
				...
				<div class="modal-dialog">
				  ...
				  <div class="modal-content">
					<div class="modal-header">
					  ...
					  <div class="col-xs-8"><h4 class="modal-title"><u>Select Elements and Layers</u></h4></div>
					  <div class="col-xs-2" style:"float:right important;">
					  <label class="checkbox-inline"><input type="checkbox" id="ret"><I>Retail</I></label>
					  </div>
					   <div class="col-xs-2"><button type="button" class="close" data-dismiss="modal">&times;</button></div>
					</div>
